...
class VcpkgPackageRegistry(PackageRegistry):
    ...
    ) -> GitHub.GitHubApi.GitHubRepoInfo:
    ...
    @staticmethod
    def get_github_latest_commit_info(
        github_user: str, github_repo: str, ref: str
    ) -> GitHub.GitHubApi.GitHubCommitInfo:
        ref = ref or "HEAD"
    ...
    @staticmethod
    def create_portfile_contents_vcpkg_from_git(
        port_name: str,
        github_user: str,
        github_repo: str,
        ref: str,
...
"""
    ...
    @staticmethod
    def create_portfile_contents_download_latest(
        port_name: str,
        github_user: str,
        github_repo: str,
        ref: str,
...
"""
    ...
    def create_portfile_contents(
        ...
        if latest:
            return self.create_portfile_contents_download_latest(
                port_name, github_user, github_repo, ref, options_text, header_only
            )
        else:
            return self.create_portfile_contents_vcpkg_from_git(
                port_name, github_user, github_repo, ref, options_text, header_only
            )

    @staticmethod
    def create_vcpkg_json_dict(
        port_name: str,
    ...
    def add_package(
        ...
        latest_commit = self.get_github_latest_commit_info(
        ...
        portfile_contents = self.create_portfile_contents(
            port_name, github_user, github_repo, latest, ref, options, header_only
        )
        ...
        version_string = (
            "latest" if latest else f"{latest_commit.date}-{latest_commit.sha[:7]}"
        )
        vcpkg_json_dict = self.create_vcpkg_json_dict(
            port_name,
            repo_description,
            github_user,
            github_repo,
        ...
        version_json = {
            "versions": [{"version-string": version_string, "git-tree": git_tree_sha}]
        }
    ...
    def update_port(self, port_name: str, ref: str = None) -> None:
        ...
        if "vcpkg_from_git" not in portfile_contents:
        ...
        portfile_contents = portfile_contents.replace(
            f"REF {current_ref}", f"REF {latest_commit.sha}"
        )
        portfile_path = self.get_portfile_path(port_name)
        ...
        version_json_data["versions"].append(
            {"version-string": version_string, "git-tree": git_tree_sha}
        )
...
class PackageRegistryCLI:
    ...
    def parse_args(self):
        ...
        parser.add_argument(
            "--dry-run", action="store_true", help="Don't perform any git commits."
        )
        parser.add_argument(
            "-r",
            "--registry",
            default="all",
            help="Comma-separated list of registries to use (default: all)",
        )
        ...
        add_parser.add_argument(
            "--options",
            help='Comma-separated list of CMake options in the format "Option=Value"',
            default="",
        )
        add_parser.add_argument(
            "--header-only",
            action="store_true",
            help="Configure as header-only library (removes lib/debug folders)",
        )

        subparsers.add_parser("ls", help="List all ports in the registry.")

        remove_parser = subparsers.add_parser(
            "rm", help="Remove a port from the registry."
        )
        ...
        update_versions_parser = subparsers.add_parser(
            "update-versions", help="Update the versions file for a port."
        )
    ...
    def execute(self):
        ...
        selected_registries = (
            self.args.registry.split(",")
            if self.args.registry != "all"
            else self.registries.keys()
        )
        ...
        for registry_name in selected_registries:
            ...
            if hasattr(self.args, "github_repo") and self.args.github_repo.startswith(
                "https://github.com/"
            ):
